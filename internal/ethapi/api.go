...
func (s *PublicBlockChainAPI) IsSuperNode(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (bool, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return true, err
	}
	return state.IsSuperNode(address), state.Error()
}
...
func (s *PublicBlockChainAPI) GetProof(ctx context.Context, address common.Address, storageKeys []string, blockNrOrHash rpc.BlockNumberOrHash) (*AccountResult, error) {
	...
	return &AccountResult{
		...
		IsSuper:      state.IsSuperNode(address),
	}, state.Error()
}
