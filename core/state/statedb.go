...
func (self *StateDB) AddSuperNode(addr common.Address) {
	//log.Info(fmt.Sprintf("BillHong statedb.AddVoteBalance  addr = %x, amount = %d", addr, amount)) // add BillHong
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.AddSuperNode()
	}
}
...
func (s *StateDB) Suicide(addr common.Address) bool {
	...
	stateObject.data.IsSuper= false
